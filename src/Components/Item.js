...
const Item = (props) => {
    return (
       <View style={{backgroundColor:"#ffffff", borderColor:"#E1E1DE" , borderWidth: 1 , marginTop:15, padding:10, height:185}}>
        <Text style={{marginBottom:8, fontWeight:"500"}}>Trending This Week in <Text style={{color:"#215A5A"}}>{props.category}</Text></Text>
        <View style={{flexDirection:'row'}}>
          <Image source= {props.img} style={{width:80 , height:110, marginRight:10}}/>
          <View style={{justifyContent:"space-between", flex:1}}>
            <Text style={{fontSize: 16, fontFamily:'Georgia'}}>{props.title}</Text>
            <Text style={{fontSize:12, color:"#6F6F6F"}}>by {props.author}</Text>
